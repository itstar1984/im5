...
angular.module('ventas').controller('VentasMostradorController', ['$scope', '$rootScope', '$state', '$http', '$filter', '$q', 'Authentication', 'Cajas', 'Categories', 'Condicionventas', 'Enterprises', 'Modal', 'Ventas', 'user','$mdDialog',
    function($scope, $rootScope, $state, $http, $filter, $q, Authentication, Cajas, Categories, Condicionventas, Enterprises, Modal, Ventas, user,$mdDialog) {
        ...
        $scope.$watchCollection('condicionVenta', function() {
        ...
        $scope.findCajas = function() {
            ...
            Cajas.query({ e: $scope.user.enterprise.enterprise }, function(foundCaja) {
                foundCaja.forEach(function(entry) {
                    ...
                    if (entry.puestos.length > 0 && entry.puestos.map(puesto => puesto._id).includes(user.puesto)) {
                        $scope.caja = entry;
                    }
                });
        ...
        $scope.addToList = function(product) {
            global.unit[product.name] = product.metric;
            if(product.metric == 'U.')
            {
           ...
           {
             $mdDialog.show({
                    targetEvent: document.querySelector('body'),
                    templateUrl: 'modules/ventas/views/model.quantity.select.html',
                    locals:{
                        item:product,
                        productosDeVenta:$scope.productosDeVenta,
                        calcularTotales:$scope.calcularTotales
                    },
        ...
        function DialogController($scope,$mdDialog,$state,item,productosDeVenta,calcularTotales)
        {
            $scope.item = item;
            $scope.countvalue = 0;
            $scope.productosDeVenta = productosDeVenta;
            $scope.calcularTotales = calcularTotales;
            $scope.closeDialog = function()
            {
                $mdDialog.hide();
            }

            $scope.createventas = function($event,item)
            {
                if(global.itemList.hasOwnProperty(item.name))
                {
                    global.itemList[item.name] += $scope.countvalue;
                    for (var i = 0; i < $scope.productosDeVenta.length; i++) {
                        if ($scope.productosDeVenta[i].item.name === item.name) {
                            $scope.productosDeVenta.splice(i, 1);
                            break;
                        }
                    }
                ...
                else
                {
                 ...
                 $scope.productosDeVenta.push({
                    product: item,
                    cantidad: global.itemList[item.name],
                    total: parseFloat(global.itemList[item.name]) * parseFloat(item.price)
                });
                $scope.calcularTotales();
                $mdDialog.hide();
                global.total = global.total + parseFloat(item.price) * $scope.countvalue;
            }
        }
        ...
        function createVenta(venta) {
            ...
            venta.user = $scope.authentication.user._id;
