...
var _ = require('lodash'),
    ...
    commonFunc = require(path.resolve(  './modules/core/common/common.server.function'));
                    ...
                    venta.products.forEach(function (entry) {
                        var productTask = function (productCallback) {
                            productoConId(entry, function (err, foundProduct) {
                                ...
                                commonFunc.getStockByUserId(venta.user, foundProduct, function(err, idxStock){
                                    if (err)
                                        return res.status(200).send({ message: "can't search userstock" });
                                    if(idxStock!= null){
                                        var nuevoStock = foundProduct.stocks[idxStock].unitsInStock - entry.cantidad;
                                        var produccionTasks = [];
                                            ...
                                            foundProduct.produccion.forEach(function (produccion) {
                                                ...
                                                var produccionTask = function (produccionCallback) {
                                                    materiaConId(produccion.producto, function (err, producto) {
                                                        ...
                                                        commonFunc.getStockByUserId(venta.user, producto, function(err, idxStock1){
                                                            if (!err && idxStock1 != null){
                                                                producto.stocks[idxStock1].unitsInStock = parseFloat(producto.stocks[idxStock1].unitsInStock - totalCant).toFixed(2);
                                        ...
                                        fall(produccionTasks, function (err) {
                                            ...
                                            foundProduct.stocks[idxStock].unitsInStock = nuevoStock;
                                            /* if (nuevoStock >= 0) {
    ...
    if (venta.estado == 'Anulada') {
        ...
        for (var i = 0; i < venta.products.length; i++) {
            ...
            productoConId(venta.products[i], function (c) {
                ...
                commonFunc.getStockByUserId(venta.user, venta.products[i], function(err, idxStock){
                    if (err)
                        return res.status(200).send({ message: "can't search userstock" });

                    if(idxStock!= null){
                        var actualStock = c.stocks[idxStock].unitsInStock;
                        ...
                        if (produccion.length > 0) {
                            for (var i = 0; i < produccion.length; i++) {
                                ...
                                commonFunc.getStockByUserId(venta.user, venta.products[i], function(err, idxStock1){
                                    if(!err && idxStock1 != null){
                                        var stockA = prod.stocks[idxStock1].unitsInStock;
                                        var num = parseFloat(stockA) + parseFloat(totalCant);
                                        prod.stocks[idxStock1].unitsInStock = num;
                                    }
                                    n.producto = prod;
                        ...
                        c.stocks[idxStock].unitsInStock = parseInt(actualStock) + parseInt(pedidos);
                        c.save(function (err) {
                            if (err) {
...
exports.list = function (req, res) {
    ...
    if (enterprise !== null) {
        ...
        Venta.find({
            ...
            estado: estado,
            deleted: false
        }).skip(pagina).limit(limite).sort('-created').populate('condicionVenta', 'name').populate('user').populate('enterprise', 'name').populate('cliente', 'name address phone').populate('category1', 'name').exec(function (err, ventas) {
            if (err) {
                return res.status(400).send({ message: errorHandler.getErrorMessage(err) });
            } else {
                var end = new Date();
        ...
        }).limit(100).sort('-created').populate('condicionVenta', 'name').populate('user').populate('enterprise', 'name').populate('cliente', 'name address phone').populate('category1', 'name').exec(function (err, ventas) {
            if (err) {
                return res.status(400).send({ message: errorHandler.getErrorMessage(err) });
            } else {
                var end = new Date();
...
exports.loadMore = function (req, res) {
    ...
    const { filter, limit, pagina:skip } = aqp(req.query);
    if (last) {
        Venta.find(filter)
            .skip(skip)
            .limit(limit)
...
exports.select = function (req, res) {
    ...
    if (enterprise !== null) {
        Venta.find({
            ...
            estado: estado,
            deleted: false
        }).limit(10).populate('user').populate('enterprise', 'name').populate('cliente', 'name address phone').populate('category1', 'name').exec(function (err, ventas) {
            if (err) {
                return res.status(400).send({ message: errorHandler.getErrorMessage(err) });
            } else {
                var end = new Date();
...
exports.ventaByID = function (req, res, next, id) {
    Venta.findById(id).populate('user').populate('enterprise', 'name').populate('tipoComprobante').populate('cliente', 'name address phone').populate('category1', 'name').populate('condicionVenta', 'name').populate('caja', 'name').exec(function (err, venta) {
        if (err)
            return next(err);
        if (!venta)
...
function productosDeCliente(clientId, callback) {
    Cliente.findById(clientId).populate('user').populate('enterprise', 'name').populate('sub', 'name').populate('contacts').populate('taxCondition', 'name').exec(function (err, cliente) {
        if (err)
            return callback(err);
...
function productoConId(productObject, callback) {
    Product.findById(productObject.product._id).populate('user').populate('enterprise', 'name').populate('produccion.producto').exec(function (err, product) {
        if (err)
            return callback(err);
...
function materiaConId(productId, callback) {
    Product.findById(productId).populate('user').populate('enterprise', 'name').exec(function (err, product) {
        if (err)
            return callback(err);
        callback(null, product)
    });
