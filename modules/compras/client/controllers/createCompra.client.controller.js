angular.module('compras').controller('CreateCompraController', ['$scope', '$rootScope', '$stateParams', '$location', 'Authentication', 'Compras', 'Enterprises','$mdBottomSheet', '$state', '$mdDialog', '$filter', 'compras', 'Products', 'Modal', 'Contacts', 'Taxconditions', 'Categories', 'Subs', 'Metrics', 'Providers', '$http', 'Cajas', 'Impuestos', 'tipoCompra',
	function($scope, $rootScope, $stateParams, $location, Authentication, Compras, Enterprises, $mdBottomSheet, $state, $mdDialog, $filter, compras, Products, Modal, Contacts, Taxconditions, Categories, Subs, Metrics, Providers, $http, Cajas, Impuestos, tipoCompra) {
		...
		$scope.cajaId = new URLSearchParams(window.location.search).get('cajaId');
		$scope.$watchCollection('condicionV', function(){
			if($scope.condicionV !== undefined){
				$scope.verCondicionVenta($scope.condicionV);
			}
		});
		...
		$scope.goback = function()
		{
			if($scope.cajaId)
			{
				window.location.href = '/cajas/view/' + $scope.cajaId;	
			}
			else
			{
		...
		$scope.$watchCollection('compraVerificada', function (){
		});

		$rootScope.$watchCollection('nuevoProveedor', function(){
		...
		$rootScope.$watchCollection('nuevaMateriaPrima', function(){
		...
		$scope.searchTextChangeProduct = function(text){
			...
			if ($scope.idProveedor === 0){
				$scope.filtrados = $filter('filter')($scope.initproduct, function(item){
			...
			return $scope.filtrados = $filter('filter')($scope.initproduct, {name: text});
		}; //end searchTextChangeProduct

		$scope.searchTextChange = function(text){
			var lowercaseQuery = angular.lowercase(text);
			return $filter('filter')($scope.initprovider, {name: text});
		}; //end searchText
		...
		$scope.selectedItemChangeProduct = function (item){
			...
			if (($scope.producto !== null) && ($scope.producto !== undefined)){
				...
				if(!$scope.producto.provider[0].id)
				{
					var id = '';
					for(var item_provider in $scope.producto.provider[0])
					{
						if(!isNaN(item_provider))
						{
							id += $scope.producto.provider[0][item_provider];
						}
					}
				...
				{
					for(var index=0;index<$scope.producto.provider.length;index++)
					{
						id_array.push($scope.producto.provider[index].id);
					}
	
				}
				
				$scope.initprovider = [];
				for(index=0;index<$scope.proveedores.length;index++)
				{
					if(id_array.indexOf($scope.proveedores[index]._id) > -1)
					{
						$scope.initprovider.push($scope.proveedores[index]);
					}
				}
			}
			else
			{
				$scope.initprovider = $scope.proveedores;
			}
		}
		...
		$scope.selectedItemChange = function (item) {
			...
			if(item!==null){
				...
				{
					if($scope.products[index].provider.length > 0)
					{
						if(!$scope.products[index].provider[0].id)
						{
							var id ='';
							for(var item_provider in $scope.products[index].provider[0])
							{
								if(!isNaN(item_provider))
								{
									id += $scope.products[index].provider[0][item_provider];
								}
							}

							if(id == item._id)
							{
								$scope.initproduct.push($scope.products[index]);
							}
							continue;
						}
						for(var index_provider = 0;index_provider<$scope.products[index].provider.length;index_provider++)
						{
							if($scope.products[index].provider[index_provider].id && $scope.products[index].provider[index_provider].id == item._id)
							{
								$scope.initproduct.push($scope.products[index]);
								break;
							}
			...
			{
				$scope.initproduct = $scope.products;
			}
        ...
        $scope.agregarProducto = function(producto,productoCompra) {
			...
			if (producto != undefined){
				...
				p.product = producto;
				if(producto.provider[0].id)
				{
					for(var item in producto.provider)
					{
						if(producto.provider[item].price)
						{
							p.product.costPerUnit = producto.provider[item].price;
						}

						if(producto.provider[item].tax)
						{
							p.product.tax = producto.provider[item].tax;
						}
					}
				}
		...
		$scope.create = function(e) {
			if($scope.puesto != undefined){
				...
				if (this.condicionV && this.condicionV.name == 'Cuenta Corriente'){
					caja = 0;
				}
										...
										if ($scope.caja !== undefined || $scope.caja !== ''){
											if(this.condicionV !== undefined){
												...
												var compra = new Compras ({
													...
													tipoComprobante: this.tipoComprobante._id,
													products: $rootScope.productosAgregados,
													proveedor: $scope.proveedor.id,
		...
		$scope.findProductos = function() {
			if($scope.SEARCH !== undefined) {
				Products.query({ e: $scope.SEARCH.enterprise}).$promise.then(function(product){
		...
		$scope.findProveedores = function() {
			if($scope.SEARCH !== undefined) {
				Providers.query({ e: $scope.SEARCH.enterprise }).$promise.then(function(provider){
					$scope.proveedores = provider;	
					$scope.initprovider = $scope.proveedores;
