...
module.exports = function(io, socket) {
    ...
    socket.on('arqueo.create', function(message) {
        ...
        if(message.operacion == 'Apertura de Caja' || message.operacion == 'Cierre de Caja')
        {
            Caja.findById(messagedata.caja,function(err,caja){
                var date = new Date();
                caja.updated = date;
        ...
        createArqueo(messagedata, function(result){
            if(result.status !== 'success') {
